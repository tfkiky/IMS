...
* Ver    变更日期             负责人  变更内容
* ───────────────────────────────────
* V0.01  2016/10/17 21:30:22   N/A    初版
*
	...
	public partial class SMT_TIMESCALE_INFO
	{
		...
		public decimal Add(Maticsoft.Model.SMT_TIMESCALE_INFO model)
		{
			...
			SqlParameter[] parameters = {
					...
					new SqlParameter("@TIME_RANGE_START1", SqlDbType.DateTime,5),
					new SqlParameter("@TIME_RANGE_END1", SqlDbType.DateTime,5),
					new SqlParameter("@TIME_RANGE_START2", SqlDbType.DateTime,5),
					new SqlParameter("@TIME_RANGE_END2", SqlDbType.DateTime,5),
					new SqlParameter("@TIME_RANGE_START3", SqlDbType.DateTime,5),
					new SqlParameter("@TIME_RANGE_END3", SqlDbType.DateTime,5),
		...
		public Maticsoft.Model.SMT_TIMESCALE_INFO DataRowToModel(DataRow row)
		{
			...
			if (row != null)
			{
				...
				{
					model.TIME_RANGE_START1=DateTime.Parse(row["TIME_RANGE_START1"].ToString());
				}
				if(row["TIME_RANGE_END1"]!=null && row["TIME_RANGE_END1"].ToString()!="")
				{
					model.TIME_RANGE_END1=DateTime.Parse(row["TIME_RANGE_END1"].ToString());
				}
				if(row["TIME_RANGE_START2"]!=null && row["TIME_RANGE_START2"].ToString()!="")
				{
					model.TIME_RANGE_START2=DateTime.Parse(row["TIME_RANGE_START2"].ToString());
				}
				if(row["TIME_RANGE_END2"]!=null && row["TIME_RANGE_END2"].ToString()!="")
				{
					model.TIME_RANGE_END2=DateTime.Parse(row["TIME_RANGE_END2"].ToString());
				}
				if(row["TIME_RANGE_START3"]!=null && row["TIME_RANGE_START3"].ToString()!="")
				{
					model.TIME_RANGE_START3=DateTime.Parse(row["TIME_RANGE_START3"].ToString());
				}
				if(row["TIME_RANGE_END3"]!=null && row["TIME_RANGE_END3"].ToString()!="")
				{
					model.TIME_RANGE_END3=DateTime.Parse(row["TIME_RANGE_END3"].ToString());
				}
